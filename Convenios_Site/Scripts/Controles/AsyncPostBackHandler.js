/*
 ...
 Version: 06-04-10
	...
	Customizacion
	=============	
	aspbh_exceptions: array de controles sobre los cuales no se hace el display de la capa
	
	Changelog
	=========
  < 06-04-10 - Original (muestra capa de para que no se ejecuten multiples eventos con ajax
	06-04-10 - Se agrego soporte a controles con excepciones
...
if (!document.getElementById("errorMessageLabel"))
	document.write("<div id='errorMessageLabel' style='color:red;'></div>");

var divMsg = "<div style=''";
divMsg += "<div style=''";
divMsg += "<div id='msgPageRequestManagerHandler' style='";
...
divMsg += "'><br/><br/>Cargando, Aguarde...</div></div></div>";
...
//	msg.style.left = document.body.clientWidth/2 - (msg.style.width.replace("px","")/2) +"px";
//	msg.style.top = document.documentElement.scrollTop + document.body.scrollTop + document.body.clientHeight/2 - (msg.style.height.replace("px","")/2) + "px";
	msg.style.left = document.body.clientWidth / 2 - (msg.style.width.replace("px", "") / 2) + "px";
	msg.style.top = document.body.clientHeight / 4 - (msg.style.height.replace("px", "") / 2) + "px";
